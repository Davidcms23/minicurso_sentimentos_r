...
Você pode encontrar o ID de um livro buscando no site [gutenberg.org](https://www.gutenberg.org/browse/languages/pt) ou usando a função `gutenberg_works(language = "pt")` no console.

```{r download}
# 1. Defina o ID do livro que você escolheu
id_livro <- 10

# 2. Baixe o livro
livro_bruto <- gutenberg_download(id_livro) 

# 3. Criamos uma coluna 'linha' para servir como nosso eixo temporal
livro_bruto <- livro_bruto %>%
  mutate(linha = row_number())

# Verifique se o download funcionou
head(livro_bruto)
```
...
```{r temporal}
# 1. Calcular Sentimento (no texto bruto)
sentimentos <- get_sentiment(livro_bruto$text, 
                             method = "syuzhet", 
                             language = "portuguese")

# 2. Agrupar por blocos (ajuste o divisor se o livro for muito curto ou longo)
...
ggplot(livro_arco, aes(x = tempo, y = saldo)) +
  ...
  labs(title = "Arco Narrativo da Giblibia",
       y = "Saldo Emocional",
       x = "Por 100 linhas") +
  theme_minimal()
```
